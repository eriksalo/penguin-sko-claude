...
function Lines({ lines, accent }: { lines: BomLine[]; accent: 'yellow' | 'ice' }) {
  return (
      ...
      <tbody className="divide-y divide-penguin-lighter/50">
        {lines.map((l) => (
          <tr key={l.name}>
            ...
            <td className={cn('py-1.5 pl-2 text-right tabular font-semibold whitespace-nowrap', accent === 'yellow' ? 'text-yellow' : 'text-ice')}>
...
export function BomTable({ result }: { result: Comparison }) {
  ...
  const row = (label: string, a: React.ReactNode, b: React.ReactNode, strong = false) => (
    <tr className={cn(strong && 'bg-penguin-light/60')}>
      ...
      <td className={cn('py-3 px-3 text-right tabular', strong ? 'text-lg md:text-xl font-black text-yellow' : 'text-snow')}>{a}</td>
  ...
  return (
    <div className="space-y-5">
        ...
        <table className="w-full text-sm md:text-base">
          <thead>
            <tr className="border-b border-penguin-lighter">
              ...
              <th className="py-3 px-3 text-right text-yellow font-black uppercase tracking-wide">VDURA Mixed Fleet</th>
          ...
          <tbody className="divide-y divide-penguin-lighter/50 [&_td]:px-3 [&_th]:px-3">
            {row(
              'Architecture',
              <span>{vdura.velos} VELO + {vdura.vpods} VPOD ({vdura.ssdSizeTB}TB TLC){vdura.jbods > 0 && <> + {vdura.jbods} JBOD</>}</span>,
      ...
      <div className="rounded-2xl border border-yellow/40 bg-gradient-to-br from-yellow/15 to-yellow/5 p-5 text-center">
        <p className="text-xs font-semibold uppercase tracking-widest text-muted">Storage savings with Penguin + VDURA</p>
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="rounded-2xl border border-yellow/30 bg-penguin/70 p-4">
              <h4 className="mb-2 text-sm font-black uppercase tracking-wide text-yellow">VDURA Mixed Fleet</h4>
              <Lines lines={vdura.lines} accent="yellow" />
            </div>
            <div className="rounded-2xl border border-ice/30 bg-penguin/70 p-4">
              <h4 className="mb-2 text-sm font-black uppercase tracking-wide text-ice">All-Flash Competitor (C Box + D Box)</h4>
